...
FROM `rakaminkfanalytics-455004.kimia_farma.kf_final_transaction`
GROUP BY year
ORDER BY year;
...
FROM `rakaminkfanalytics-455004.kimia_farma.kf_final_transaction` t
JOIN `rakaminkfanalytics-455004.kimia_farma.kf_product` p 
...
SELECT 
    ...
    COUNT(*) AS total_terjual
FROM `rakaminkfanalytics-455004.kimia_farma.kf_final_transaction` t
JOIN `rakaminkfanalytics-455004.kimia_farma.kf_kantor_cabang` p 
...
FROM `rakaminkfanalytics-455004.kimia_farma.kf_final_transaction` AS t
JOIN `rakaminkfanalytics-455004.kimia_farma.kf_kantor_cabang` AS p
...
SELECT
    ...
    t.provinsi,
    t.rating
FROM `rakaminkfanalytics-455004.kimia_farma.kf_product` AS p
CROSS JOIN `rakaminkfanalytics-455004.kimia_farma.kf_kantor_cabang` AS t;
...
SELECT DISTINCT 
    ...
    t.product_name,
    t.opname_stock
FROM `rakaminkfanalytics-455004.kimia_farma.kf_inventory` AS t
JOIN `rakaminkfanalytics-455004.kimia_farma.kf_final_transaction` AS p
ON t.product_id = p.product_id
WHERE t.opname_stock <> 0
ORDER BY p.date DESC;

---total transaksi di indonesia selama 2020-2023
SELECT
